...
>
> **Vocalmax — 10 từ kỹ thuật (đọc to, ghi âm, so sánh phát âm):**  **bytecode** /ˈbaɪtkoʊd/ — mã trung gian biên dịch từ source code **classloader** /ˈklɑːsloʊdər/ — bộ nạp class vào JVM **heap** /hiːp/ — vùng nhớ chứa object **stack** /stæk/ — vùng nhớ chứa stack frame của thread **metaspace** /ˈmetəspeɪs/ — vùng chứa class metadata **JIT compiler** /dʒɪt kəmˈpaɪlər/ — biên dịch nóng bytecode → native **interpreter** /ɪnˈtɜːprətər/ — thông dịch bytecode từng lệnh **runtime** /ˈrʌntaɪm/ — thời điểm chương trình đang chạy **native** /ˈneɪtɪv/ — mã máy của nền tảng cụ thể **execution engine** /ˌeksɪˈkjuːʃn ˈendʒɪn/ — bộ máy thực thi bytecode  **Mock Interview EN — trả lời to, ghi âm 60-90s:** *"How does the JVM run a Java program?"* **Gợi ý trả lời:** "When I run a Java program, the compiler first turns my `.java` source into platform-independent `.class` bytecode. At runtime, the JVM's classloader subsystem loads these class files using a delegation model, then links and initializes them. The bytecode lives in the runtime data areas — the heap holds objects, each thread gets its own stack, and class metadata sits in the metaspace. The execution engine reads the bytecode: it starts by interpreting instructions one by one, and the JIT compiler kicks in for hot methods, compiling them into optimized native code so the program runs much faster. This is what gives Java its 'write once, run anywhere' property — the same bytecode runs on any JVM."
...
>
> **STAR Method Practice** — luyện kể chuyện kỹ thuật theo cấu trúc **S**ituation – **T**ask – **A**ction – **R**esult. Câu hỏi behavioral: *"Tell me about a time you debugged a difficult performance or memory issue."* **Khung trả lời STAR (ghi âm 90-120s):**  **Situation:** "In a previous project, our backend service started crashing in production with OutOfMemoryError after running for a few hours under load." **Task:** "I was responsible for finding the root cause and preventing the crashes without a full rewrite." **Action:** "I enabled heap dumps on OOM, analyzed them with a memory profiler, and found a static cache that kept growing because entries were never evicted. I replaced it with a bounded cache and added weak references for cached objects." **Result:** "Memory usage stabilized, the service ran for weeks without restarts, and I documented the pattern so the team avoided it elsewhere."  Lưu ý: nói **"I"** không phải "we" khi mô tả Action; Result phải có con số hoặc kết quả cụ thể.
...
> 🌏 **TIẾNG ANH — HỌC TRƯỚC (trên app)** · *Mock Interview EN · 20 phút*
>
> **Mock Interview EN** — câu hỏi kỹ thuật, trả lời tiếng Anh, ghi âm 90s: *"Can you explain how garbage collection works in Java, and what GC algorithm you'd choose for a low-latency service?"* **Gợi ý:** "Java's garbage collector automatically reclaims memory from objects that are no longer reachable from GC roots. It works in phases — mark live objects, sweep dead ones, and compact to avoid fragmentation. It relies on the generational hypothesis: most objects die young, so it collects the young generation frequently and cheaply with minor GCs, and the old generation less often. For a low-latency service, I'd choose **ZGC** or **Shenandoah** because they do most of their work concurrently and keep pause times in the single-digit milliseconds even with large heaps. **G1** is the balanced default that works well for most applications, while **Parallel GC** maximizes throughput but has longer stop-the-world pauses."
...
> 🌏 **TIẾNG ANH — HỌC TRƯỚC (trên app)** · *Mock Interview EN · 20 phút*
>
> **Mock Interview EN** — trả lời tiếng Anh, ghi âm: *"What design patterns do you use most often, and can you give a concrete example?"* **Gợi ý:** "I use creational patterns a lot. The **Builder** pattern is my go-to for objects with many optional fields — it makes the construction readable and the object immutable, like building an HTTP request or a configuration object. I rely on the **Factory** pattern to decouple object creation from usage, so the calling code depends on an interface, not a concrete class. For shared, single-instance services I use **Singleton**, though in Spring I mostly let the framework manage singleton beans for me. The key for me is using a pattern only when it solves a real problem — overusing them leads to over-engineering."
...
> 🌏 **TIẾNG ANH — HỌC TRƯỚC (trên app)** · *Mock Interview EN + Vocalmax · 15 phút*
>
> **Vocalmax — 8 từ về design patterns (đọc to, ghi âm):**  **adapter** /əˈdæptər/ — bộ chuyển đổi interface **decorator** /ˈdekəreɪtər/ — thêm hành vi động **strategy** /ˈstrætədʒi/ — thuật toán hoán đổi được **observer** /əbˈzɜːrvər/ — đối tượng lắng nghe sự kiện **behavior** /bɪˈheɪvjər/ — hành vi **interchangeable** /ˌɪntərˈtʃeɪndʒəbl/ — có thể hoán đổi **subscribe** /səbˈskraɪb/ — đăng ký nhận thông báo **polymorphism** /ˌpɒliˈmɔːfɪzəm/ — tính đa hình  **Mock Interview EN:** *"Can you explain the Strategy pattern and when you'd use it instead of if-else?"* **Gợi ý:** "The Strategy pattern defines a family of interchangeable algorithms behind a common interface, so I can swap the behavior at runtime. For example, for payment processing I'd define a `PaymentStrategy` interface with implementations like `CreditCardStrategy` and `PayPalStrategy`. I'd choose it over a long if-else chain when the branches represent distinct algorithms that change independently — it follows the open-closed principle, so I add a new payment method by writing a new class instead of editing existing code."
...
>
> **Full Mock Interview EN** — chủ đề code quality & design. Trả lời tiếng Anh, ghi âm cả buổi:  *"How do you keep a codebase clean and maintainable as it grows?"* *"How do you approach refactoring a large legacy class safely?"* *"Can you explain the SOLID principles and how you apply them?"*  **Gợi ý cho câu refactoring (STAR-friendly):** "I never refactor without a safety net. First I make sure there are tests around the code — if there aren't, I write characterization tests that capture the current behavior. Then I refactor in small steps, running the tests after each change, so I can catch regressions immediately. I look for code smells like long methods and duplicate code, and I apply techniques like extract method, extract class, and replacing conditionals with polymorphism. The goal is to improve the structure without changing the observable behavior."
...
>
> **Comprehensive Mock Interview** — kết hợp technical + behavioral, mô phỏng vòng phỏng vấn thật. Trả lời tiếng Anh, ghi âm và nghe lại:  *Technical:* "Walk me through how the JVM manages memory and garbage collection." *Technical:* "Explain a design pattern you've used and why you chose it." *Behavioral (STAR):* "Tell me about a time you made an important design decision and how it played out." *Behavioral (STAR):* "Describe a time you mentored someone or influenced the team's technical direction."  **Mẹo:** với câu technical, cấu trúc câu trả lời rõ ràng (overview → chi tiết → ví dụ → trade-off). Với câu behavioral, bám sát STAR và kết bằng kết quả đo được. Ghi âm toàn bộ, nghe lại để sửa filler words ("um", "like") và tốc độ nói.
