...
## 10. Cải tiến bổ sung #7–9 (ĐÃ XONG — 2026-07-11)

Ba cải tiến "đóng vòng" để khóa không chỉ *đo* mà còn *ép đúng* và *nối người thật*:

### #7 — Chống điểm ảo (voice-first Boss + đối chiếu transcript)

- **Buổi Boss BẮT BUỘC nói bằng giọng**: `aiContext.voiceRequired = d.isBoss`. `useChatEngine`
  có `voiceRequired` (chỉ ép khi trình duyệt có STT — Safari/iOS không khóa cứng kẻo kẹt).
  `ChatComposer` khóa ô gõ + nút Gửi khi `voiceRequired`; `send({viaVoice})` chặn câu gõ tay,
  chỉ nhận câu đến từ mic.
- **Đối chiếu WPM/pronScore với transcript**: `fluencyStats.SPOKEN_WPM_CEILING=220` +
  `isSuspiciousWpm` + `fluencyIntegrity(samples,userTurns)` → `{userTurns,voiceTurns,suspiciousTurns}`.
  Lượt "nói" mà WPM vượt trần = transcript dài hơn thời gian nói thật (nghi dán chữ).
- **Cảnh báo khi lệch**: `integrityWarning` (client) hiện banner ở `AiChat` khi Boss mà chưa nói
  bằng giọng lần nào / có lượt WPM bất thường. Đồng thời đẩy `integrity`+`voiceRequired` vào
  context debrief → `buildDebriefPrompt` thêm `integrityLine` (LLM không thổi phồng trôi
  chảy/dễ hiểu, nhắc phải trả lời bằng giọng để có điểm thật).

### #8 — Remediation cá nhân hóa theo confusions THẬT

- **Track confusions theo nhóm âm**: `PronunciationDrill` emit `mpAttempt {group,confused}` mỗi
  lần thử cặp tối thiểu (STT thật); `pairsForWeek` trả thêm `pairGroups[]` để quy lỗi đúng nhóm.
  Store `recordCommConfusion(groupKey,confused)` đếm dồn qua cả khóa vào `convoPrefs.commConfusions`.
- **Đẩy drill đúng chỗ yếu**: `focusForWeek`/`pairsForWeek` nhận `priorityKeys` (dùng đúng nhóm đó
  thay lịch tuần cứng); `PronunciationDrill` thêm `priorityGroups` + chế độ `pairsOnly`.
  `commWeakPairGroups(user)` suy nhóm hay lẫn (đủ số lần thử + tỉ lệ ≥ ngưỡng).
- **"Phục thù phát âm"**: `CommDayView` hiện block `pairsOnly` nhắm các âm yếu NGOÀI trọng tâm tuần
  (buổi mission: mọi âm yếu) — remediation theo dữ liệu thật, không theo lịch cứng.

### #9 — Giao thức người thật có phản hồi

- **Rubric chuẩn tự/peer chấm**: `PeerRubricSheet.vue` — 4 chiều (dễ hiểu / trôi chảy / từ vựng-ngữ
  pháp / mạch lạc), thang 1–4, 2 cột Tự chấm + Người nghe; lưu `convoPrefs.commPeerRubric`.
- **Chia sẻ bản ghi self-intro**: `recordingSync.createRecordingShareLink` (signed URL 7 ngày) +
  mẫu lời nhờ nhận xét (kèm 4 chiều rubric) copy/Web-Share sẵn → dán ra cộng đồng. Gắn ở
  `CommSummaryView` (block "🤝 Xin nhận xét từ người thật"). Không tự đăng gì — chỉ tạo link.

**Tests:** `fluencyStats` (ceiling/integrity), `minimalPairs` (priorityKeys/pairGroups),
`commStats` (commWeakPairGroups), `recordingSync` (createRecordingShareLink), `user.store`
(recordCommConfusion). **685 test pass, `vite build` OK.** Route comm cần auth + mic + cloud nên
chưa tự test trình duyệt (giống các đợt trước).
