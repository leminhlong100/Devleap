# Tuần 2 — Clean Sentence Engine (Cấu trúc mệnh đề cơ bản)

## 🎯 Mục tiêu & Trọng tâm tuần

- **Mục tiêu tuần:** Em biết câu tiếng Anh cần xương sống gì.
- **Trọng tâm ngữ pháp:** subject, verb, object/complement, be vs action verbs, singular/plural
- **Nhóm từ vựng:** People, family, school, home
- **Speaking:** Answer Part 1 questions with 2–3 sentences
- **Writing:** Sentence garden: 5 daily sentences
- **Reading:** identify who/what/where/when
- **Listening:** catch basic verbs and nouns
- **Nhiệm vụ nhẹ:** Mô tả căn phòng như một cảnh phim.
- **Mốc kiểm tra:** Sửa được lỗi thiếu động từ như She beautiful.

### Nhịp học hằng ngày

| Day | Nhiệm vụ chính | Sản phẩm nhỏ | Ôn tập |
| --- | --- | --- | --- |
| Day 8 | Chủ ngữ – động từ hợp nhau | 8 câu có động từ | Nhớ-lại 3 cụm Tuần 1 |
| Day 9 | Đếm được / không đếm (much/many) | 6 câu much/many | Đọc to câu Day 8 |
| Day 10 | Phòng từ vựng Family & School | 10 thẻ cụm | Anh→Việt→Anh |
| Day 11 | Nói Part 1 (2–3 câu) | Bản ghi 60–90s | Lật Flashcard |
| Day 12 | Ngày nạp: mô tả căn phòng | 6 câu "phim hóa" phòng | Đọc lại câu Part 1 |
| Day 13 | Vườn câu 8 câu sạch lỗi | 3 câu ghi âm | Xem sổ lỗi tuần |
| Day 14 | Tổng hợp + kiểm tra tuần | Bài kiểm tra Tuần 2 | Nhớ-lại quy tắc much/many |

### Quyền học chậm

Trong tuần này, người học có quyền đi chậm, lặp lại và dùng tiếng Việt để hiểu ý trước khi diễn đạt bằng tiếng Anh. Điều cần giữ là nhịp quay lại đều đặn trong cả tuần.

**Lưu ý:** Đi chậm vẫn có giá trị nếu người học quay lại, sửa lỗi và dùng lại kiến thức.

## 📖 Ngữ pháp trọng tâm

### Câu phải có động từ — Chủ ngữ hợp động từ

Một câu tiếng Anh phải có động từ, và động từ phải hợp với chủ ngữ. Tiếng Việt nói được 'Cô ấy đẹp', 'Các bạn tôi ở rạp phim' mà không cần 'là/ở'; tiếng Anh thì luôn cần **be** (is/am/are) hoặc một động từ rõ ràng. Ngoài ra, chủ ngữ số nhiều thì danh từ cần thêm 's' và động từ KHÔNG thêm 's'.

**Mẫu câu chính:** Subject + (be / động từ) + Object/Complement

**Câu đúng:**

- My mother is very patient.
- The streets are crowded at night.
- My friends are at the cinema.
- Students need to review regularly.

**Lỗi thường gặp của người học Việt Nam:**

| Câu sai | Câu đúng |
| --- | --- |
| She beautiful. | She is beautiful. |
| My friends at the cinema. | My friends are at the cinema. |
| Student need to review. | Students need to review. |
| She like music. | She likes music. |

**Bài tập ngắn:** Thêm động từ còn thiếu vào 5 câu: 1) My mother very patient. 2) The streets crowded at night. 3) My phone on the table. 4) Many students afraid of speaking. 5) Learning English a long journey.

**Mẹo nhớ:** Viết xong câu, tự hỏi "Động từ của câu này đâu?" — nếu câu mô tả tính chất / nơi chốn mà không thấy động từ thường thì chắc chắn cần **be** (is/am/are). Chủ ngữ số nhiều ("students", "friends") đi với động từ nguyên thể, nhưng danh từ số nhiều phải có "s".

### Countable và Uncountable

English rất quan tâm đến số lượng. Vietnamese often does not mark plural clearly, so learners forget s/es or use much/many wrongly.

**Mẫu câu chính:**

| Cấu trúc | Dùng với |
| --- | --- |
| many + plural count nouns | danh từ đếm được số nhiều |
| much + uncountable | danh từ không đếm được |
| a lot of | cả hai |

**Câu đúng:**

- I have many friends.
- I do not have much free time.
- There is a lot of information online.
- Students need useful resources.

**Lỗi thường gặp của người học Việt Nam:**

| Câu sai | Câu đúng |
| --- | --- |
| I have many homework. | I have a lot of homework. |
| many information | a lot of information / much information |
| two advice | two pieces of advice |
| There is many students in my class. | There are many students in my class. |

**Bài tập ngắn:** Lập hai danh sách: 10 danh từ đếm được và 10 danh từ không đếm được trong đời sống hằng ngày.

**Mẹo nhớ:** "information", "advice", "homework", "water" là danh từ KHÔNG đếm được — không thêm "s", dùng **much / a lot of** chứ không dùng "many". Cần đếm thì mượn đơn vị: *two pieces of advice*.

## 🏋️ Ngân hàng luyện tập — câu mẫu sạch lỗi

> Đây đều là câu ĐÚNG để đọc to & shadowing (Day 12) và làm mẫu cho "vườn câu" (Day 13).
> Mỗi câu đã qua 3 lần tự kiểm: có động từ chưa? "s" số nhiều đúng chưa? đếm được/không đếm được đúng chưa?

*Gia đình & quan hệ (be · hòa hợp chủ ngữ–động từ):*

1. My mother is very patient.
2. My friends are at the cinema.
3. My brothers are still at school.
4. I am close to my sister because she listens to me.
5. In many Vietnamese families, children respect their parents a lot.
6. My parents share the housework every evening.

*Trường lớp & học tập:*

7. Learning English is a long journey.
8. Many students are afraid of speaking.
9. A good teacher makes a difficult subject easier to understand.
10. Students who review regularly usually remember longer.
11. Making mistakes is part of learning a language.
12. She likes music very much.

*Đếm được / không đếm được (much · many · a lot of):*

13. I have a lot of homework this week.
14. I do not have much free time.
15. There is a lot of information online.
16. I do not have much experience with IELTS yet.
17. She gave me two useful pieces of advice.
18. There are many students in my class.

*There is / There are · mô tả nơi chốn:*

19. There is a park near my house.
20. There is a wooden desk near the window.
21. There are two large windows in the living room.
22. My phone is on the table.
23. The streets are crowded at night.
24. The weather is getting colder these days.

## 🗂️ Phòng từ vựng

### Family and Relationships

**Từ chính:** parents, siblings, relatives, support, argument, generation, care, respect, responsibility, childhood

**Cụm dùng được:**

- spend time with family
- be close to
- get along with
- share housework
- look after

**Câu nối với IELTS:**

- I am quite close to my mother because she listens to me.
- In many Vietnamese families, children respect their parents a lot.
- Sharing housework can make family life more comfortable.

### School and Learning

**Từ chính:** subject, teacher, classmate, homework, exam, mistake, method, memory, skill, improvement

**Cụm dùng được:**

- pay attention in class
- build a habit
- review regularly
- ask for feedback
- learn from mistakes

**Câu nối với IELTS:**

- A good teacher can make a difficult subject easier to understand.
- Students should review regularly instead of cramming before exams.
- Making mistakes is part of learning a language.

## 🛠️ Kỹ năng & Khung mẫu

### Cấu tạo câu (Writing)

Mỗi câu có các thành phần chính.

| Thành phần | Nội dung |
| --- | --- |
| Core | Subject + Verb |
| Support | object/complement |
| Time | yesterday/every day/now/in the future |
| Place | at school/in my city/online |
| Reason | because... |
| Example | for example... |
| Feeling/result | so I feel.../it helps... |

Người học bắt đầu bằng một câu cơ bản:

> Students use phones.

Thêm chi tiết:

> Many students use phones every day.

Thêm lý do:

> Many students use phones every day because they need to study and communicate.

Thêm ý cân bằng:

> Many students use phones every day because they need to study and communicate, but phones can also distract them.

Bài viết phát triển bằng cách mở rộng từng bước, không phải bằng cách học thuộc những mở bài phức tạp nhưng xa ý thật.

## 📅 Lịch học 7 ngày (Daily Checklist)

> Mỗi buổi ~45–60 phút. Không bỏ *Ôn đầu giờ* và *Sổ lỗi*. Trọng tâm tuần: xương sống câu — chủ ngữ–động từ hợp nhau, đếm được/không đếm được, bồi câu ngắn thành câu dài.

### Day 8 — Chủ ngữ – động từ hợp nhau (be vs động từ thường)
- [ ] Ôn (5'): nhớ-lại 3 cụm của Tuần 1.
- [ ] Học: điểm ngữ pháp của buổi; chú ý "đã có động từ chưa?" và "s" số nhiều.
- [ ] Luyện ngữ pháp: làm bài tập (thêm động từ còn thiếu), đạt ≥70%.
- [ ] Từ vựng: học từ mới + nghe 🔊, lưu Flashcard.
- [ ] Viết: viết 8 câu về gia đình/lớp học, tự kiểm động từ; nhờ AI chữa.
- [ ] Sổ lỗi (5'): câu nào suýt quên động từ?

### Day 9 — Đếm được / Không đếm được (much / many / a lot of)
- [ ] Ôn (5'): đọc to 5 câu của Day 8.
- [ ] Học: much + không đếm được, many + số nhiều; "information/advice/homework" không đếm được.
- [ ] Luyện ngữ pháp: làm bài tập, đạt ≥70%.
- [ ] Viết: làm bài viết của buổi + 6 câu dùng much/many/a lot of; nhờ AI chữa.
- [ ] Nói & Quiz: luyện hội thoại với AI + quiz từ vựng.
- [ ] Sổ lỗi (5'): ghi lỗi "many information / two advice".

### Day 10 — Phòng từ vựng Family & School
- [ ] Ôn (5'): nhớ-lại quy tắc much/many bằng tiếng Việt.
- [ ] Luyện ngữ pháp: làm lại bài tập điểm đang ôn, đạt ≥70%.
- [ ] Từ vựng: học theo cụm (get along with, review regularly, learn from mistakes…), nghe 🔊, lưu Flashcard.
- [ ] Viết: dùng 3 "Câu nối IELTS" làm khung, viết 6 câu thật; nhờ AI chữa.
- [ ] Nói & Quiz: luyện hội thoại với AI + quiz từ vựng.
- [ ] Sổ lỗi (5'): cụm nào dễ quên?

### Day 11 — Luyện nói Part 1 (trả lời 2–3 câu)
- [ ] Ôn (5'): lật thẻ Flashcard (Anh→Việt→Anh).
- [ ] Học: nhận diện lỗi câu trước khi nói (sửa câu mẫu trong trang).
- [ ] Luyện ngữ pháp: làm bài tập, đạt ≥70%.
- [ ] Viết: làm bài viết của buổi; nhờ AI chữa.
- [ ] Nói: hỏi–đáp 5 câu về gia đình/trường lớp với AI, mỗi câu 2–3 câu trả lời (ghi âm 60–90s).
- [ ] Sổ lỗi (5'): lỗi lặp lại khi nói?

### Day 12 — Ngày nạp: "mô tả căn phòng như một cảnh phim"
- [ ] Ôn (5'): đọc to phần trả lời Part 1 của Day 11.
- [ ] Học: cách bồi câu (Core → Support → Time → Place → Reason → Example).
- [ ] Luyện ngữ pháp: làm bài tập, đạt ≥70%.
- [ ] Viết: "phim hóa" căn phòng — viết 6 câu (there is/are + be); nhờ AI chữa.
- [ ] Nói & Quiz: nói với AI mô tả phòng + quiz từ vựng.
- [ ] Sổ lỗi (5'): 2 cụm mới muốn dùng lại.

### Day 13 — Ngày sản phẩm: "vườn câu" 8 câu sạch lỗi
- [ ] Ôn (5'): xem nhanh sổ lỗi cả tuần.
- [ ] Học: ôn quy trình bồi câu; chọn 2 chủ đề (family, school).
- [ ] Luyện ngữ pháp: làm bài tập, đạt ≥70%.
- [ ] Viết: viết 8 câu, mỗi câu tự kiểm 3 điểm (động từ? "s"? đếm được?); nhờ AI chữa.
- [ ] Sản phẩm: chọn 3 câu hay nhất, đọc to & ghi âm.
- [ ] Sổ lỗi (5'): sửa lại 3 câu sai nhất trong tuần.

### Day 14 — Tổng hợp tuần + Bài kiểm tra tuần
- [ ] Ôn (10'): nhớ-lại 5 cụm + quy tắc much/many.
- [ ] Luyện ngữ pháp: làm bài tập tổng hợp cuối tuần, đạt ≥70%.
- [ ] Viết: viết 8 câu vận dụng các điểm ngữ pháp tuần; nhờ AI chữa.
- [ ] Quiz: làm quiz từ vựng của buổi.
- [ ] Kiểm tra: làm Bài kiểm tra Tuần 2 (đạt ≥70% để mở tuần sau).
- [ ] Kế hoạch: chọn 3 nội dung cần lặp lại.

## ✅ Quiz tuần 2

### 🧠 Quiz Nhanh

1. Chọn câu đúng về danh từ không đếm được?
   - [ ] I have many informations.
   - [ ] I have many information.
   - [x] I have a lot of information.
   - [ ] I have two informations.
   💡 "Information" là danh từ không đếm được, không thêm "s" và dùng "a lot of" hoặc "much".

2. Điền từ đúng: "I do not have ______ free time."?
   - [ ] many
   - [x] much
   - [ ] a many
   - [ ] manys
   💡 "Free time" không đếm được nên dùng "much", không dùng "many".

3. Câu nào đúng về động từ "be"?
   - [ ] She beautiful.
   - [x] She is very patient.
   - [ ] She are patient.
   - [ ] She be patient.
   💡 Câu cần động từ "be"; chủ ngữ số ít "she" đi với "is".

4. Chọn câu đúng về sự hòa hợp chủ ngữ – động từ?
   - [ ] My friends is at the cinema.
   - [ ] Student need to review.
   - [x] Students need to review regularly.
   - [ ] My friends at the cinema.
   💡 Chủ ngữ số nhiều "students/friends" đi với động từ nguyên mẫu, và "students" cần "s".

5. Điền đúng: "There ______ many students in my class."?
   - [ ] is
   - [x] are
   - [ ] be
   - [ ] has
   💡 "Many students" là số nhiều nên dùng "There are".

### Part A — Fix the sentences

1. My mother very patient.
2. I have many homework.
3. There is many students in my class.
4. She like music very much.
5. I am go to school every day.

### Part B — Complete the answers

1. In my family, there are ______.
2. I get along with ______ because ______.
3. A good teacher should ______.
4. I do not have much ______.
5. I have a lot of ______.

### Part C — Speaking

Answer in 60 seconds:

- Describe your family: how many people, and what they do.

### Answer Key — Part A

1. My mother is very patient.
2. I have a lot of homework.
3. There are many students in my class.
4. She likes music very much.
5. I go to school every day.

### Teacher note

Nếu người học vẫn lặp lại cùng một lỗi sau khi được sửa, đừng nói "we learned this already". Hãy nói: "This pattern needs more review. Let's repair it again with a new sentence."
